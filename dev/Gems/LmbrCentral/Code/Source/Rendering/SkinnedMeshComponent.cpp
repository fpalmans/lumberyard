...
namespace LmbrCentral
{
    ...
    bool SkinnedMeshComponentRenderNode::SkinnedRenderOptions::VersionConverter(AZ::SerializeContext& context,
    ...
    const char* SkinnedMeshComponentRenderNode::GetJointNameByIndex(AZ::u32 jointIndex) const
    {
        if (m_characterInstance)
        {
            const IDefaultSkeleton& skeleton = m_characterInstance->GetIDefaultSkeleton();
            ...
            {
                return skeleton.GetJointNameByID(jointIndex);
            }
        }
    ...
    AZ::u32 SkinnedMeshComponentRenderNode::GetJointIndexByName(const char* jointName) const
    {
        if (m_characterInstance)
        {
            ...
            if (jointIndex >= 0)
            {
                return static_cast<AZ::u32>(jointIndex);
            }
        }
    ...
    AZ::Transform SkinnedMeshComponentRenderNode::GetJointTransformCharacterRelative(AZ::u32 jointIndex) const
    {
        if (m_characterInstance)
        {
            const IDefaultSkeleton& skeleton = m_characterInstance->GetIDefaultSkeleton();
            if (jointIndex < skeleton.GetJointCount())
            {
                ISkeletonPose* skeletonPose = m_characterInstance->GetISkeletonPose();
                if (skeletonPose)
                {
                    return LYQuatTToAZTransform(skeletonPose->GetAbsJointByID(jointIndex));
                }
            }
        }
    ...
    void SkinnedMeshComponentRenderNode::OnCharacterBoundsReset()
    {
        UpdateLocalBoundingBox();
    }
    ...
    void SkinnedMeshComponentRenderNode::OnAssetReady(AZ::Data::Asset<AZ::Data::AssetData> asset)
    {
        ...
        if (asset.Get() == m_characterDefinitionAsset.Get())
        {
            ...
            m_characterInstance = m_characterDefinitionAsset.Get()->m_characterInstance;

            if (m_characterInstance != nullptr)
            {
                if (!AZ::CharacterBoundsNotificationBus::Handler::BusIsConnectedId(m_characterInstance.get()))
                {
                    AZ::CharacterBoundsNotificationBus::Handler::BusConnect(m_characterInstance.get());
                }
            }
    ...
    /*IRenderNode*/ _smart_ptr<IMaterial> SkinnedMeshComponentRenderNode::GetEntitySlotMaterial(unsigned int nPartId, bool bReturnOnlyVisible, bool* pbDrawNear)
    {
        if (0 == nPartId)
        {
            return m_materialOverride;
    ...
    namespace ClassConverters
    {
        ...
        static bool DeprecateMeshComponent(AZ::SerializeContext& context, AZ::SerializeContext::DataElementNode& classElement)
        {
            ...
            if (!path.empty())
            {
                assetData = AZ::Data::AssetManager::Instance().GetAsset(meshAssetId, meshAssetUuId);
            }
