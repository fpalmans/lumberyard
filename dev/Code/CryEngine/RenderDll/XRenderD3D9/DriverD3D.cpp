...
void CD3D9Renderer::Release()
{
    ...
    // GPUParticleEngine resources are released as part of shutdown logic
    // above. GPUParticleEngine does not get deleted at that same moment
    // for map-change situations; we delete here as we are shutting down
    // the renderer completely.
    SAFE_DELETE(m_gpuParticleEngine);
...
//////////////////////////////////////////////////////////////////////////
bool CD3D9Renderer::ScreenShotInternal(const char* filename, int iPreWidth)
{
...
void CD3D9Renderer::ReadFrameBufferFast(uint32* pDstARGBA8, int dstWidth, int dstHeight, bool BGRA)
{
...
unsigned int CD3D9Renderer::DownLoadToVideoMemory(const byte* data, int w, int h, int d, ETEX_Format eTFSrc, ETEX_Format eTFDst, int nummipmap, ETEX_Type eTT, bool repeat, int filter, int Id, const char* szCacheName, int flags, EEndian eEndian, RectI* pRegion, bool bAsyncDevTexCreation)
{
    FUNCTION_PROFILER_FAST(GetISystem(), PROFILE_RENDERER, g_bProfilerEnabled);
